 ...
 function DovellousClassConstructor(Framework7Class) {
    return class Dovellous extends Framework7Class {
      ...
      renderSheet() {
        ...
        const sheetHtml = `
          <div class="sheet-modal dovellous dovellous-sheet dovellous-type-${
            dovellous.params.type
          } ${cssClass || ''}">
        ...
        const popoverHtml = `
          <div class="popover dovellous-popover">
            <div class="popover-inner">
              <div class="dovellous dovellous-type-${dovellous.params.type} ${
          cssClass || ''
        }">
                ${toolbar ? dovellous.renderToolbar() : ''}
      ...
      renderInline() {
        ...
        const inlineHtml = `
          <div class="dovellous dovellous-inline dovellous-type-${dovellous.params.type} ${
          cssClass || ''
        }">
            ${toolbar ? dovellous.renderToolbar() : ''}
            <div class="dovellous-buttons">
  ...
  const DovellousF7MobilePlugin = {
    ...
    create() {
      ...
      app.dovellous = {
        create(params) {
          return new Dovellous(app, params);
        },
        get(el = '.dovellous') {
          if (el instanceof Dovellous) return el;
          const $el = $(el);
          if ($el.length === 0) return undefined;
          return $el[0].f7Keypad;
        },
        destroy(el = '.dovellous') {
          const instance = app.dovellous.get(el);
        ...
        close(el = '.dovellous') {
          ...
          const dovellous = $el[0].f7Keypad;
          if (!dovellous || (dovellous && !dovellous.opened)) return;
          dovellous.close();
        },
      };
    },
